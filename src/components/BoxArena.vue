<template>
  <svg
    ...
    :viewBox="viewBox"
  >
    <rect class="ground" :style="{ width: mapWidth, height: mapHeight }" />
    <BoxyBot :player="p1" />
    <BoxyBot :player="p2" />
    ...
    <g clip-path="url(#p1H-clip)">
      ...
      <path id="p1-health" class="health-bar fill" :style="{ d: `path('m4 4h${Math.max(p1.h, 0)}')` }" />
    </g>
    ...
    <g clip-path="url(#p2H-clip)">
      ...
      <path id="p2-health" class="health-bar fill" :style="{ d: `path('m124 4h-${Math.max(p2.h, 0)}')` }" />
    </g>
  </svg>
</template>
